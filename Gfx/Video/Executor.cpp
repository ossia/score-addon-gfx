...
public:
  ...
  {
    return "Gfx::video_node";
  }
};

ProcessExecutorComponent::ProcessExecutorComponent(
    Gfx::Video::Model& element, const Execution::Context& ctx,
    const Id<score::Component>& id, QObject* parent)
    : ProcessComponent_T{element, ctx, id, "gfxExecutorComponent", parent}
{
  auto n = std::make_shared<video_node>(element.decoder(), ctx.doc.plugin<DocumentPlugin>().exec);
